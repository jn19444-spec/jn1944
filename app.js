...
async function routeFromLocation() {
  ...
  if (path === "/hovercell") {
    if (currentUser) selectHovercell({ skipUrl: true });
    else replaceUrl("/");
    return;
  }
...
onAuthStateChanged(auth, async (user) => {
  ...
  el("hovercellBtn").classList.toggle("hidden", !user);
  if (!user && !el("hovercellView").classList.contains("hidden")) {
    // 호버셀 제작 툴을 보고 있는데 로그아웃(또는 승인 취소)되면 홈으로 돌려보내요.
    showListView();
    showHomeDashboard();
    replaceUrl("/");
  }
...
// ---------- 호버셀 제작 툴 (승인된 회원만, 별도 툴을 iframe으로 연동) ----------
function selectHovercell(opts = {}) {
  if (!currentUser) { replaceUrl("/"); return; } // 로그인/승인된 회원이 아니면 접근 불가
  currentBoardId = null;
  currentBoard = null;
  el("writeBtn").classList.add("hidden");
  showHovercellView();
  ...
  if (!frame.src) frame.src = "hovercell/index.html"; // 처음 열 때만 로드(불필요한 재로딩 방지)
  if (!opts.skipUrl) pushUrl("/hovercell");
}
el("hovercellBtn").addEventListener("click", () => selectHovercell());
